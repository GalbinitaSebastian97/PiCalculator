...
Author: Galbinita Sebastian
Algoithm: Chudnovsky Algorithm
Module Dependencies:
...
def getIteratedValue(k):
	...
	k = k+1
	##2.15
	getcontext().prec = k ##set the precision of digits which Decimal lib will use to return any calculated number, that is, prec = 2 will return 2 digits (first=2, second=2) and in prec = 3 that will show three numbers(first=2, second=1, third=5).
	sum=0
	## This is the Chidnovsky series
	for k in range(k):
...
def shell():
	...
	while True:
		print (">>> ", end='')
		entry = input() ## is the input you type
		if entry == "quit":
			break
